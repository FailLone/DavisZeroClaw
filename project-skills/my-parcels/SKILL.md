---
name: my-parcels
description: Check the user's recent parcels from the locally connected Alibaba/Taobao and JD order pages through the Davis localhost proxy. Use when the user asks about recent deliveries, packages in transit, whether anything is arriving today, whether there are pending pickup parcels, or wants to search parcels by platform, product name, or status. Do not ask the user for a tracking number by default.
---
